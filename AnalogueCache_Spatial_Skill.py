...
if 'ciclad' in myhost:
    ...
    # Jasmin options
    forecast_datadir = '/work/scratch-nopw/mmenary/CMIP_{:s}/'.format(forecast_var)
    analogue_datadir = '/work/scratch-nopw/mmenary/CMIP_{:s}/'.format(analogue_var)
    processed_output_dir = '/work/scratch-nopw/mmenary/AnalogueCache'
    scripts_dir = '/home/users/mmenary/scripts/'
    hadisst_save_file = '/home/users/mmenary/data/HadISST_time_series_regions.pkl'
    en4_save_file = '/home/users/mmenary/data/EN4_0-500m_time_series_regions.pkl'
    hadcrut4_save_file = '/home/users/mmenary/data/HadCRUT4_time_series_regions.pkl'
...
id = hashlib.md5(processed_base).hexdigest()
...
if residual:
    ...
    with open(forecast_save_file, 'rb') as handle:
        ...
        _, _, _, year_forecast_obs = pickle.load(handle)
    ...
    with open(analogue_save_file, 'rb') as handle:
        ...
        _, _, _, year_analogue_obs = pickle.load(handle)
    ...
    for tt, year in enumerate(year_forecast_obs):
        ...
        for imem in range(num_mems_to_take):
            ...
            with open(this_file, 'rb') as handle:
                _, sst_ts_in, _, _, _, year_in = pickle.load(handle)
            ...
            if analogue_var != forecast_var:
                ...
                with open(this_file2, 'rb') as handle:
                    _, analogue_ts, _, _, _, year_in_analogue = pickle.load(handle)
...
for ifile, pf in enumerate(processed_files):
    ...
    with open(pf, 'rb') as handle:
        corr_annual, corr_trend, _, year_model = pickle.load(handle)
